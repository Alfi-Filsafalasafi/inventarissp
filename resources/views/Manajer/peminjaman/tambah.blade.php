      ...
      <div class="row">
        ...
        <div class="col-md-12">
          ...
          <div class="box box-success">
            ...
            <form action="{{route('peminjaman.storebarang.manajer', ['kode_pinjam'=>$kode_pinjam])}}" method="POST" enctype="multipart/form-data">
                    ...
                    <table id="example1" class="table table-bordered table-hover">
                        ...
                        @forelse ($barangpinjam as $bp)
                            <tr>
                              ...
                              <td>{{$bp->jumlah}}</td>
                              <td><a href="{{route('peminjaman.deletebarangpinjam.manajer',['peminjaman'=>$bp->id, 'kode_pinjam' => $kode_pinjam])}}" class="btn btn-danger btn-xs show_confirm" style="margin: 2px;"><i class="fa fa-trash"></i></a></td>
                            </tr>   
                        @empty
...
<!-- Data Peminjam -->
            ...
            <form action="{{route('peminjaman.finalisasi.manajer',['kode_pinjam' => $kode_pinjam]) }}" method="POST" enctype="multipart/form-data">
            @method('PATCH')
            @csrf
            <div class="box-body">
...
<script>
  $('#lokasi').change(function(){
    ...
    if(id_lokasi){
        $.ajax({
           url:"/manajer/getbarang?id_lokasi="+id_lokasi,
           method:"GET",
           dataType: 'json',
...
<script>
  $('#barang').change(function(){
    ...
    if(id_barang){
        $.ajax({
           url:"/manajer/gettersedia?id_barang="+id_barang,
           method:"GET",
           dataType: 'json',
           success:function(res){               
            if(res){
